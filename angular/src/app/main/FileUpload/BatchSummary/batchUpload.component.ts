...
@Component({
  ...
  styleUrls: ['./batchUpload.component.less'],
})
export class BatchUploadComponent extends AppComponentBase {
  @ViewChild('ExcelFileUpload', { static: false }) excelFileUpload: FileUpload;
  public dateRange: DateTime[] = [this._dateTimeService.getStartOfDay(), this._dateTimeService.getEndOfDay()];
  // invoices:StandardFileReport[];
  invoices: any[] = [];
  tenantId: Number;
  tenantName: String;
  fromDate: Date;
  ...
  getBatchUploadData() {
    this._fileUpload.getReportData(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString())).subscribe((result) => {
  ...
  exportToExcel(fileName: string, batchId: number): void {
    this._fileUpload
      .getInvalidRecordsToExcel(batchId, fileName)
      .subscribe((result) => {
