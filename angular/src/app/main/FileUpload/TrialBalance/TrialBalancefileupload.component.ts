...
@Component({
  templateUrl: './TrialBalancefileupload.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./TrialBalancefileupload.component.less'],
})
export class TrialBalanceUploadComponent extends AppComponentBase {
  ...
  constructor(
    ...
    super(injector);
    this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/TrialBalanceFileUpload/ImportFromExcel';
  }
  ...
  loadNotifications(): void {
    ...
    this._notificationService.getUserNotifications(undefined, undefined, undefined, 3, 0).subscribe((result) => {
      ...
      _forEach(result.items, (item: UserNotification) => {
        ...
        if (this._userNotificationHelper.format(<any>item).text.startsWith('Trial Balance File Upload')) {
  ...
  uploadExcel(data: { files: File }): void {
    ...
    this._httpClient
      ...
      .subscribe((response) => {
        if (response.success) {
          this.isLoading = true;
          this.notify.success(this.l('ImportTrialBalanceProcessStart'));
        } else if (response.error != null) {
          this.notify.error(this.l('ImportTrialBalanceUploadFailed'));
        }
      });
  }
