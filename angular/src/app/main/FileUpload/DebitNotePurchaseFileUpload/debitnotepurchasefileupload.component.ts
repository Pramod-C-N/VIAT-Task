...
@Component({
  templateUrl: './debitnotepurchasefileupload.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./debitnotepurchasefileupload.component.less'],
})

export class NewFileDebitPurchaseBatchUploadComponent extends AppComponentBase {
  ...
  constructor(
    ...
    super(injector);
    this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/DebitNotePurchaseFileUpload/ImportFromExcel';
  }
  ...
  loadNotifications(): void {
    ...
    this._notificationService.getUserNotifications(undefined, undefined, undefined, 3, 0).subscribe((result) => {
      ...
      _forEach(result.items, (item: UserNotification) => {
        ...
        if (this._userNotificationHelper.format(<any>item).text.startsWith('Debit Purchase File Upload')) {
  ...
  uploadExcel(data: { files: File }): void {
    ...
    this._httpClient
      ...
      .subscribe((response) => {
        if (response.success) {
          this.isLoading = true;
          this.notify.success(this.l('ImportDebitNotePurchaseProcessStart'));
        } else if (response.error != null) {
          this.notify.error(this.l('ImportDebitNotePurchaseUploadFailed'));
        }
      });
  }
