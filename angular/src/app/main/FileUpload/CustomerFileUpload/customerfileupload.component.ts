...
@Component({
  templateUrl: './customerfileupload.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./customerfileupload.component.less'],
})
export class CustomerUploadComponent extends AppComponentBase {
  ...
  constructor(
    ...
    super(injector);
    this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/CustomerFileUpload/ImportFromExcel';
  ...
  loadNotifications(): void {
    ...
    this._notificationService.getUserNotifications(undefined, undefined, undefined, 3, 0).subscribe((result) => {
      ...
      _forEach(result.items, (item: UserNotification) => {
        ...
        if (this._userNotificationHelper.format(<any>item).text.startsWith('Customer File Upload')) {
  ...
  uploadExcel(data: { files: File }): void {
    ...
    this._httpClient
      ...
      .subscribe((response) => {
        if (response.success) {
          this.isLoading = true;
          this.notify.success(this.l('ImportCustomerProcessStart'));
        } else if (response.error != null) {
          this.notify.error(this.l('ImportCustomerUploadFailed'));
        }
      });
  }
