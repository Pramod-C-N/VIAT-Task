...
import { SalesInvoicesServiceProxy, NotificationServiceProxy, UserNotification, AllowanceReasonDto } from '@shared/service-proxies/service-proxies';
...
import { ImportBatchDataDto, ImportBatchDatasServiceProxy } from '@shared/service-proxies/service-proxies';
import { DateTime } from 'luxon';
import { DateTimeService } from '@app/shared/common/timing/date-time.service';

@Component({
  templateUrl: './purchasefileupload.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./purchasefileupload.component.less'],
})
export class NewFilePurchaseBatchUploadComponent extends AppComponentBase {
  ...
  constructor(
    ...
    super(injector);
    this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/PurchaseNoteFileUpload/ImportFromExcel';
  }
  ...
  loadNotifications(): void {
    ...
    this._notificationService.getUserNotifications(undefined, undefined, undefined, 3, 0).subscribe((result) => {
      ...
      _forEach(result.items, (item: UserNotification) => {
        ...
        if (this._userNotificationHelper.format(<any>item).text.startsWith('Purchase File Upload')) {
  ...
  uploadExcel(data: { files: File }): void {
    ...
    this._httpClient
      ...
      .subscribe((response) => {
        if (response.success) {
          this.isLoading = true;
          this.notify.success(this.l('ImportPurchaseNoteProcessStart'));
        } else if (response.error != null) {
          this.notify.error(this.l('ImportPurchaseNoteUploadFailed'));
        }
      });
  }
