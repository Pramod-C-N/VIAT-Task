...
@Component({
  templateUrl: './creditnotepurcasefileupload.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./creditnotepurcasefileupload.component.less'],
})
export class NewFileCreditPurchaseBatchUploadComponent extends AppComponentBase {
  ...
  constructor(
    ...
    super(injector);
    this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/CreditNotePurchaseFileUpload/ImportFromExcel';
  }
  ...
  ngOnInit(): void {
    ...
    this._salesInvoicesAppService.getFileMappings().subscribe((e) => {
      console.log(e);
      if (e) {
          this.mappingList = e.filter(v=>v.transactionType=="Purchase(Credit)").map(v=>{return {name:v.name,id:v.id}});
      }
  });
  }
  view() {
  ...
  uploadExcel(data: { files: File }): void {
    ...
    this._httpClient
      ...
      .subscribe((response) => {
        if (response.success) {
          this.isLoading = true;
          this.notify.success(this.l('ImportCreditNotePurchaseProcessStart'));
        } else if (response.error != null) {
          this.notify.error(this.l('ImportCreditNotePurchaseUploadFailed'));
        }
      });
  }
