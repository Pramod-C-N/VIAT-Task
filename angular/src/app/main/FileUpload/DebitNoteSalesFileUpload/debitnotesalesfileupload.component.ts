...
@Component({
  templateUrl: './debitnotesalesfileupload.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./debitnotesalesfileupload.component.less'],
})
export class NewFileDebitSalesBatchUploadComponent extends AppComponentBase {
  ...
  constructor(
    ...
    super(injector);
    this.uploadUrl = AppConsts.remoteServiceBaseUrl + '/DebitNoteSalesFileUpload/ImportFromExcel';
  }
  ...
  loadNotifications(): void {
    ...
    this._notificationService.getUserNotifications(undefined, undefined, undefined, 3, 0).subscribe((result) => {
      ...
      _forEach(result.items, (item: UserNotification) => {
        ...
        if (this._userNotificationHelper.format(<any>item).text.startsWith('Debit Note Sales File Upload')) {
  ...
  uploadExcel(data: { files: File }): void {
    ...
    this._httpClient
      ...
      .subscribe((response) => {
        if (response.success) {
          this.isLoading = true;
          this.notify.success(this.l('ImportDebitNoteSalesProcessStart'));
        } else if (response.error != null) {
          this.notify.error(this.l('ImportDebitNoteSalesUploadFailed'));
        }
      });
  }
  ...
  onUploadExcelError(): void {
    this.notify.error(this.l('ImportDebitNoteUploadFailed'));
  }
