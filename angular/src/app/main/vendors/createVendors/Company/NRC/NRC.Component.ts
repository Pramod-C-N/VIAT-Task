...
export class NRCComponent extends AppComponentBase {
  @Input() id: number;
  ...
  isSaving: boolean;
  basicForm: FormGroup;
  loadNrcdt: boolean = false;
  loadNrcadd: boolean = false;
  loadNrcReg: boolean = false;
  tenantForm() {
  ...
  show(CustomerId?: number): void {
    if (!CustomerId) {
      ...
      this._customerServiceProxy.getVendorForEdit(CustomerId).subscribe((result) => {
        ...
        this.customer.constitutionType = 'Non-Resident Company'
  ...
  loadgeneral() {
    this.loadNrcdt = true;
    this.loadNrcadd = false;
    this.loadNrcReg = false;
  }
  loadAdd() {
    this.loadNrcdt = false;
    this.loadNrcadd = true;
    this.loadNrcReg = false;
  }
  loadReg() {
    this.loadNrcdt = false;
    this.loadNrcadd = false;
    this.loadNrcReg = true;
  }
}