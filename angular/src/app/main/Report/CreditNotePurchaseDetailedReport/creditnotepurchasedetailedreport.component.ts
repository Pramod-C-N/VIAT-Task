...
@Component({
  templateUrl: './creditnotepurchasedetailedreport.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./creditnotepurchasedetailedreport.component.css'],
})
export class CreditNotePurchaseDetailedReportComponent extends AppComponentBase {
  ...
  ngOnInit(): void {
    ...
    // this.code = 'creditdetailed'; need in the future
  }
  ...
  getCreditNotePurchaseDaywiseReport() {
    this.isdisable = true;
    this._creditNoteProxy.getCreditPurchaseDaywiseReport(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString()))
      .pipe(finalize(() => this.isdisable = false))
      .subscribe((result) => {
        this.invoices = result;
        this.ResetData();
  ...
  changeTab(tab: string) {
    ...
    if (tab === 'Daywise') {
      ...
      this.getCreditNotePurchaseDaywiseReport();
    } else if (tab === 'Detailed') {
      this.columns = this.detailedColumns;
      this.getCreditNotePurchaseDetailedReport();
    }
  }
  searchData() {
