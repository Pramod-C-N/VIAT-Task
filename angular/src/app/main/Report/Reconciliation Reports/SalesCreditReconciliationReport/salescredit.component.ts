...
@Component({
  ...
  styleUrls: ['./salescredit.component.less'],
})
export class salescreditreportComponent extends AppComponentBase {
  ...
  getSalesDetailedReport(){
      this._VatReportServiceProxy.getSalesCreditReconciliationReport(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString())).subscribe((result) => {
      this.data = result;
     this.searched = true;
    });
  }
  downloadExcel(){
    this._VatReportServiceProxy.getSalesCreditReconciliationReportToExcel(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString()), 'SalesCreditNoteReconciliationReport', this.tenantName, false).subscribe((result) => {
