...
@Component({
  ...
  styleUrls: ['./purchasecredit.component.less'],
})

export class purchasecreditreportComponent extends AppComponentBase {
  ...
  getSalesDetailedReport() {
    this._VatReportServiceProxy.getPurchaseCreditReconciliationReport(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString())).subscribe((result) => {
      this.data = result;
      this.searched = true;
    });
  }
  downloadExcel() {
    this._VatReportServiceProxy.getPurchaseCreditReconciliationReportToExcel(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString()), 'PurchaseCreditNoteReconciliationReport', this.tenantName, false).subscribe((result) => {
