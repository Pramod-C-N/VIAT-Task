...
@Component({
  templateUrl: './salesdebit.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./salesdebit.component.less'],
})
export class salesdebitreportComponent extends AppComponentBase {
...
//ngoninit
  ...
  getSalesDetailedReport(){
      this._VatReportServiceProxy.getSalesDebitReconciliationReport(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString())).subscribe((result) => {
      this.data = result;
     this.searched = true;
    });
  }
  downloadExcel(){
    this._VatReportServiceProxy.getSalesDebitReconciliationReportToExcel(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString()), 'SalesDebitNoteReconciliationReport', this.tenantName, false).subscribe((result) => {
