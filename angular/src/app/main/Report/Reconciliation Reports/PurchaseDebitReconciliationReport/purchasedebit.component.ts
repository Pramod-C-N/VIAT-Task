...
@Component({
  templateUrl: './purchasedebit.component.html',
  encapsulation: ViewEncapsulation.None,
  animations: [appModuleAnimation()],
  styleUrls: ['./purchasedebit.component.less'],
})
export class purchasedebitreportComponent extends AppComponentBase {
  ...
  getSalesDetailedReport() {
    this._VatReportServiceProxy.getPurchaseDebitReconciliationReport(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString())).subscribe((result) => {
      console.log(result);
      this.data = result;
      console.log(this.data[0].amount);
      this.searched = true;
    });
  }

  downloadExcel() {
    this._VatReportServiceProxy.getPurchaseDebitReconciliationReportToExcel(this.parseDate(this.dateRange[0].toString()), this.parseDate(this.dateRange[1].toString()), 'PurchaseDebitNoteReconciliationReport', this.tenantName, false).subscribe((result) => {
