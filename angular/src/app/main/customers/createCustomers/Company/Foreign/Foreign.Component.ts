...
export class ForeignComponent extends AppComponentBase {
  @Input() id : number;
  ...
  isSaving: boolean;
  basicForm: FormGroup;
  loadforeigndt:boolean=false;
  loadforeignadd:boolean=false;
  loadforeignReg:boolean=false;
  vatid:string;
  uniqueidvat:string;
  ...
  tenantForm() {
    this.basicForm = this.fb.group({
            ...
            Validators.pattern("^[0-9]*$") ] ],
              ...
              nameoflegalrep: ['', Validators.required ],
              Email: ['', []],
            });
...
show(CustomerId?: number): void {
  if (!CustomerId) {
   ...
   this.mapFormtoDto();
     ...
     this.customer.tenantType = 'Company';//this.individualRequiredForm.get('firstName').value || " ";
     this.customer.constitutionType = 'Foreign Branch' //this.individualRequiredForm.get('firstName').value || " ";
     this.customer.contactNumber = this.basicForm.get('ContactNo').value || " ";//this.individualRequiredForm.get('firstName').value || " ";
     ...
     this.foreign.legalRepresentative = this.basicForm.get('nameoflegalrep').value || " ";
  ...
  save(){
   ...
   loadgeneral(){
    this.loadforeigndt=true;
    this.loadforeignadd=false;
    this.loadforeignReg=false;
    
   }

   loadAdd(){
    this.loadforeigndt=false;
    this.loadforeignadd=true;
    this.loadforeignReg=false;
    }
