...
using vita.Authentication.TwoFactor;
using vita.Authentication.TwoFactor.Google;
using vita.Authorization;
using vita.Authorization.Accounts.Dto;
using vita.Authorization.Users;
using vita.MultiTenancy;
using vita.Web.Authentication.JwtBearer;
using vita.Web.Authentication.TwoFactor;
using vita.Web.Models.TokenAuth;
using vita.Authorization.Impersonation;
...
using vita.Web.Authentication.External;
using vita.Web.Common;
using vita.Authorization.Delegation;

namespace vita.Web.Controllers
{
    [Route("api/[controller]/[action]")]
    public class TokenAuthController : vitaControllerBase
    {
        ...
        private async Task<string> EncryptQueryParameters(long userId, string passwordResetCode)
        {
            ...
            var expireDate = Uri.EscapeDataString(Clock.Now.AddHours(expirationHours).ToString(vitaConsts.DateTimeOffsetFormat));
            ...
            [FromBody] ExternalAuthenticateModel model)
        {
            ...
            var loginResult = await _logInManager.LoginAsync(
                new UserLoginInfo(model.AuthProvider, externalUser.ProviderKey, model.AuthProvider),
                 await GetTenancyNameOrNull(externalUser.EmailAddress)
            );
