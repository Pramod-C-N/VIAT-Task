...
using vita.DebitNoteFileUpload;
using NPOI.SS.Formula.Functions;

namespace vita.Web.Controllers
{
    public class DebitNotePurchaseFileUploadBase : vitaControllerBase
    {
        protected readonly IBinaryObjectManager BinaryObjectManager;
        protected readonly IBackgroundJobManager BackgroundJobManager;

        protected DebitNotePurchaseFileUploadBase(
        ...
        public async Task<JsonResult> ImportFromExcel(DateTime? fromdate, DateTime? todate)
        {
            try
            {
                ...
                await BackgroundJobManager.EnqueueAsync<ImportDebitNotePurchaseFileToExcelJob, ImportUsersFromExcelJobArgs>(new ImportUsersFromExcelJobArgs
                {
                    TenantId = tenantId,
