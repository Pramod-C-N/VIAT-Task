...
{
    [AbpAuthorize(AppPermissions.Pages_PurchaseCreditNote)]
    public class PurchaseCreditNoteAppService : vitaAppServiceBase, IPurchaseCreditNoteAppService
    {
        private readonly IRepository<PurchaseCreditNote, long> _purchaseCreditNoteRepository;
        ...
        private readonly IPurchaseCreditNoteSummaryAppService _invoiceSummariesAppService;
        private readonly IPurchaseCreditNotePartyAppService _partyAppService;
        private readonly IPurchaseCreditNoteAddressAppService _invoiceAddressesAppService;
        private readonly IPurchaseCreditNoteDiscountAppService _discountAppService;
        private readonly IPurchaseCreditNoteContactPersonAppService _contactPersonsAppService;
        private readonly IPurchaseCreditNotePaymentDetailAppService _paymentDetailsAppService;
        private readonly IPurchaseCreditNoteVATDetailAppService _vatDetailsAppService;
        private readonly IPurchaseCreditNoteItemAppService _invoiceItemsAppService;
        ...
        public PurchaseCreditNoteAppService(IRepository<PurchaseCreditNote, long> purchaseCreditNoteRepository,
            ITimeZoneConverter timeZoneConverter,
             IIRNMastersAppService transactionsAppService,
            ITenantBasicDetailsAppService tenantbasicdetails,
           IPurchaseCreditNoteSummaryAppService invoiceSummariesAppService,
            IPurchaseCreditNotePartyAppService partyAppService,
            IPurchaseCreditNoteAddressAppService invoiceAddressesAppService,
            IPurchaseCreditNoteDiscountAppService discountAppService,
            IPurchaseCreditNoteContactPersonAppService contactPersonsAppService,
            IPurchaseCreditNotePaymentDetailAppService paymentDetailsAppService,
            IPurchaseCreditNoteVATDetailAppService vatDetailsAppService,
            IPurchaseCreditNoteItemAppService invoiceItemsAppService,
            IGenerateXmlAppService generateXmlAppService,
            IPdfReportAppService pdfReportAppService, IDbContextProvider<vitaDbContext> dbContextProvider)
        {
        ...
        public async Task<PagedResultDto<GetPurchaseCreditNoteForViewDto>> GetAll(GetAllPurchaseCreditNoteInput input)
        {

            var filteredPurchaseCreditNote = _purchaseCreditNoteRepository.GetAll()
            ...
            var pagedAndFilteredPurchaseCreditNote = filteredPurchaseCreditNote
                .OrderBy(input.Sorting ?? "id asc")
                .PageBy(input);

            var purchaseCreditNote = from o in pagedAndFilteredPurchaseCreditNote
                                     select new
                                     {
            ...
            var totalCount = await filteredPurchaseCreditNote.CountAsync();

            var dbList = await purchaseCreditNote.ToListAsync();
            var results = new List<GetPurchaseCreditNoteForViewDto>();

            foreach (var o in dbList)
            {
                var res = new GetPurchaseCreditNoteForViewDto()
                {
                    PurchaseCreditNote = new PurchaseCreditNoteDto
                    {
            ...
            return new PagedResultDto<GetPurchaseCreditNoteForViewDto>(
                totalCount,
                results
            );
        ...
        public async Task<GetPurchaseCreditNoteForViewDto> GetPurchaseCreditNoteForView(long id)
        {
            var purchaseCreditNote = await _purchaseCreditNoteRepository.GetAsync(id);

            var output = new GetPurchaseCreditNoteForViewDto { PurchaseCreditNote = ObjectMapper.Map<PurchaseCreditNoteDto>(purchaseCreditNote) };

            return output;
        }

        [AbpAuthorize(AppPermissions.Pages_PurchaseCreditNote_Edit)]
        public async Task<GetPurchaseCreditNoteForEditOutput> GetPurchaseCreditNoteForEdit(EntityDto<long> input)
        {
            var purchaseCreditNote = await _purchaseCreditNoteRepository.FirstOrDefaultAsync(input.Id);

            var output = new GetPurchaseCreditNoteForEditOutput { PurchaseCreditNote = ObjectMapper.Map<CreateOrEditPurchaseCreditNoteDto>(purchaseCreditNote) };

            return output;
        }

        public async Task<long> CreateOrEdit(CreateOrEditPurchaseCreditNoteDto input)
        {           
             return await Create(input);
                 
        }
        public async Task<InvoiceResponse> CreatePurchaseCreditNote(CreateOrEditPurchaseCreditNoteDto input)
        {
            ...
            await InsertPurchaseCreditReportData(data.InvoiceId);

            return data;
        }

        private async Task<InvoiceResponse> GenerateCreditNote(CreateOrEditPurchaseCreditNoteDto input)
        {
            ...
            {
                var paymentD = new CreateOrEditPurchaseCreditNotePaymentDetailDto()
                {
                    PaymentMeans = "Cash",
                    PaymentTerms = "",
                    IRNNo = invoiceno
                ...
                input.PaymentDetails = new List<CreateOrEditPurchaseCreditNotePaymentDetailDto>();
            ...
            if (input.VATDetails == null)
            {
                ...
                input.VATDetails = new List<CreateOrEditPurchaseCreditNoteVATDetailDto>();

                foreach (var item in details)
                {
                    var vatdata = new CreateOrEditPurchaseCreditNoteVATDetailDto();
        ...
        public async Task<bool> InsertPurchaseCreditReportData(long IRNNo)
        {
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.CommandText = "InsertPurchaseCreditReportData";
        ...
        public async Task<DataTable> GetPurchaseCreditData(DateTime fromDate, DateTime toDate)
        {
            ...
            DataTable dt = new DataTable(); try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.Connection = conn;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandText = "GetPurchaseCreditData";
        ...
        public async Task<DataTable> GetPurchaseDebitData(DateTime fromDate, DateTime toDate)
        {
            fromDate = _timeZoneConverter.Convert(fromDate, AbpSession.TenantId, AbpSession.UserId ?? 0) ?? fromDate;
            toDate = _timeZoneConverter.Convert(toDate, AbpSession.TenantId, AbpSession.UserId ?? 0) ?? toDate;
            DataTable dt = new DataTable(); try
            {
                ...
                using (SqlConnection conn = new SqlConnection(connStr))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        cmd.Connection = conn;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.CommandText = "getPurchaseDebitData";
                        cmd.Parameters.AddWithValue("fromDate", fromDate);
                        cmd.Parameters.AddWithValue("toDate", toDate);
                        cmd.Parameters.AddWithValue("@tenantId", AbpSession.TenantId);
                        dt.Load(cmd.ExecuteReader());
        ...
        [AbpAuthorize(AppPermissions.Pages_PurchaseCreditNote_Create)]
        protected virtual async Task<long> Create(CreateOrEditPurchaseCreditNoteDto input)
        {
            var purchaseCreditNote = ObjectMapper.Map<PurchaseCreditNote>(input);
            purchaseCreditNote.UniqueIdentifier = Guid.NewGuid();

            if (AbpSession.TenantId != null)
            {
                purchaseCreditNote.TenantId = (int?)AbpSession.TenantId;
            }

           return await _purchaseCreditNoteRepository.InsertAndGetIdAsync(purchaseCreditNote);

        }

        

        [AbpAuthorize(AppPermissions.Pages_PurchaseCreditNote_Delete)]
        public async Task Delete(EntityDto<long> input)
        {
            await _purchaseCreditNoteRepository.DeleteAsync(input.Id);
        }
