...
namespace vita.Authorization.Accounts
{
    public class AccountAppService : vitaAppServiceBase, IAccountAppService
    {
        ...
        {
            var user = await UserManager.TryGetUserWithoutSpecifyingTenant(input.EmailAddress);
            if (user == null)
            {
                return;
            }
