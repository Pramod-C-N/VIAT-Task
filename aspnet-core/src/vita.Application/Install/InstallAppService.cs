...
using vita.Migrations.Seed.Host;
...
namespace vita.Install
{
    [AbpAllowAnonymous]
    [DisableAuditing]
    public class InstallAppService : vitaAppServiceBase, IInstallAppService
    {
        private readonly AbpZeroDbMigrator<vitaDbContext> _migrator;
        ...
        {
            var connectionString = _appConfiguration[$"ConnectionStrings:{vitaConsts.ConnectionStringName}"];
        ...
        {
            EditAppSettingsjson($"ConnectionStrings:{vitaConsts.ConnectionStringName}", constring);
        }
